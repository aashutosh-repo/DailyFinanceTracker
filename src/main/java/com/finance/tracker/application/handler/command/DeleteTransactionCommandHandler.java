...
import com.finance.tracker.application.command.DeleteTransactionCommand;
...
public class DeleteTransactionCommandHandler implements CommandHandler<DeleteTransactionCommand, TransactionDTO> {

    private final FinancialTransactionRepository repository;
    private final TransactionMapper mapper;

    @Override
    public TransactionDTO handle(DeleteTransactionCommand command) {
        ...
        transaction.cancel(command.getReason());
        transaction = repository.save(transaction);

        return mapper.toDTO(transaction);
    }

    private void validateCommand(DeleteTransactionCommand command) {
        try {
