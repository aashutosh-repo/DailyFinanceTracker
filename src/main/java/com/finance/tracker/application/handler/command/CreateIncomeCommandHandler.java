...
public class CreateIncomeCommandHandler implements CommandHandler<CreateIncomeCommand, TransactionDTO> {

    private final FinancialTransactionRepository repository;
    private final TransactionMapper mapper;

    @Override
    public TransactionDTO handle(CreateIncomeCommand command) {
        validateCommand(command);

        Money money = Money.of(command.getAmount(),command.getCurrency());

        FinancialTransaction income = FinancialTransaction.createIncome(
                command.getUserId(), money, command.getTransactionDate(),
                command.getCategoryId(), command.getDescription(), command.getIncomeSource(),
    ...
    private void validateCommand(CreateIncomeCommand command) {
        try {
